...
const ventures = [
  { name: "Quantrion AI", domain: "Learning" },
  { name: "Vdoc AI", domain: "Entertainment" },
  { name: "ExoraX AI", domain: "Commerce" },
]
    ...
    <section className="relative flex min-h-screen items-center overflow-hidden">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-7xl flex-col items-center gap-10 px-5 pt-24 pb-16 sm:px-6 md:flex-row md:gap-12 lg:gap-20 lg:px-12 lg:py-24">
        ...
        <div className="hero-rise order-2 w-full flex-1 text-center md:order-1 md:text-left">
          {/* Status badge */}
          <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/[0.06] px-4 py-1.5 backdrop-blur-sm sm:mb-8 sm:gap-2.5 sm:px-5 sm:py-2">
          ...
          {/* Description */}
          <p className="mx-auto mt-6 max-w-lg text-pretty text-sm leading-relaxed text-muted-foreground sm:mt-8 sm:text-base md:text-lg lg:mx-0">
            I build applied AI systems for learning, entertainment, and commerce.
            At <span className="font-semibold text-foreground">Quantavex</span> I
            lead research and product across three platforms, taking models from
            paper to production.
          </p>

          {/* Ventures shipped */}
          <div className="mt-8 sm:mt-10">
            <p className="text-[10px] tracking-[0.2em] text-muted-foreground uppercase sm:text-xs">
              Platforms shipped
            </p>
            <div className="mt-3 flex flex-wrap items-center justify-center gap-2 sm:mt-4 sm:gap-3 lg:justify-start">
              {ventures.map((venture) => (
                <div
                  key={venture.name}
                  className="rounded-xl border border-border/50 bg-secondary/30 px-3 py-2 text-left backdrop-blur-sm sm:px-4 sm:py-2.5"
                >
                  <div className="font-mono text-xs font-semibold text-foreground sm:text-sm">
                    {venture.name}
                  </div>
                  <div className="mt-0.5 text-[10px] tracking-wider text-muted-foreground uppercase">
                    {venture.domain}
                  </div>
                </div>
              ))}
          ...
          <div className="relative mx-auto w-full max-w-none md:max-w-[360px] lg:mx-0 lg:max-w-[400px] xl:max-w-[440px]">
            ...
            <div className="relative w-full overflow-hidden rounded-2xl border border-border/40 bg-card/30 backdrop-blur-sm sm:rounded-3xl">
              <AnimatedImage
                ...
                alt="Udit Gour - AI Researcher and Founder of Quantavex"
                width={440}
                height={550}
            ...
            <div className="absolute -top-3 -right-3 flex h-14 w-14 items-center justify-center rounded-xl border border-primary/20 bg-primary/[0.08] backdrop-blur-sm sm:-top-4 sm:-right-4 sm:h-20 sm:w-20 sm:rounded-2xl">
              <span className="font-mono text-[10px] font-bold text-primary sm:text-xs">AI</span>
            </div>
          </div>
        </div>
      </div>
